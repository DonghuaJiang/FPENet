...
class fsModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_names = self.loss_filter('G_GAN', 'G_GAN_Feat', 'G_VGG', 'G_ID', 'G_Rec', 'D_GP', 'D_real', 'D_fake')

            # initialize optimizers
            params = list(self.netG.parameters())                                          # optimizer G
            self.optimizer_G = torch.optim.Adam(params, lr=opt.lr, betas=(opt.beta1, 0.999))

            params = list(self.netD1.parameters()) + list(self.netD2.parameters())         # optimizer D
    ...
    def forward(self, img_id, img_att, latent_id, for_G=False):
        ...
        fea1_fake = self.netD1.forward(img_fake)                                           # G_GAN
        ...
        if not self.opt.no_ganFeat_loss:
            ...
            for i in range(num_D):
                for j in range(0, len(fea_fake[i]) - 1):
                    loss_G_GAN_Feat += D_weights * feat_weights * self.criterionFeat(fea_fake[i][j], fea_real[i][j].detach()) * self.opt.lambda_feat

        img_fake_down = F.interpolate(img_fake, size=(112, 112))                           # G_ID
        ...
        loss_G_Rec = self.criterionRec(img_fake, img_att) * self.opt.lambda_rec            # G_Rec

        # Only return the fake_B image if necessary to save BW
        return [self.loss_filter(loss_G_GAN, loss_G_GAN_Feat, loss_G_ID, loss_G_Rec, loss_D_GP, loss_D_real, loss_D_fake), img_fake]
